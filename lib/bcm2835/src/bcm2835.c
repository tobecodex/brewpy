...
void bcm2835_peri_set_bits(
  volatile uint32_t* paddr, uint32_t value, uint32_t mask
)
{
...
uint32_t bcm2835_gpio_pad(uint8_t group)
{
  volatile uint32_t* paddr = 
    bcm2835_pads + BCM2835_PADS_GPIO_0_27/4 + group*2;
...
uint8_t bcm2835_spi_transfer(uint8_t value)
{
  ...
  // BUG ALERT: what happens if we get interupted in this section, and 
  // someone else accesses a different peripheral? 
  // Clear TX and RX fifos
...
void bcm2835_spi_transfernb(char* tbuf, char* rbuf, uint32_t len)
{
  ...
  // This is Polled transfer as per section 10.6.1
  // BUG ALERT: what happens if we get interupted in this section, 
...
void bcm2835_pwm_set_clock(uint32_t divisor)
{
  ...
  // Stop PWM clock
  bcm2835_peri_write(
    bcm2835_clk + BCM2835_PWMCLK_CNTL, 
  ...
  bcm2835_peri_write(
    bcm2835_clk + BCM2835_PWMCLK_DIV, 
    BCM2835_PWM_PASSWRD | (divisor << 12)
  );
  
  bcm2835_peri_write(
    bcm2835_clk + BCM2835_PWMCLK_CNTL, 
    BCM2835_PWM_PASSWRD | 0x11
...
void bcm2835_pwm_set_mode(uint8_t channel, uint8_t markspace, uint8_t enabled)
{
  ...
  else if (channel == 1)
  {
    ...
    // If you use the barrier here, weird things happen, and the commands
    // dont work
